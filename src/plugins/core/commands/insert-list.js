      ...
      InsertListCommand.prototype.execute = function (value) {
        function splitList(listItemElements) {
          if (listItemElements.length > 0) {
            var newListNode = scribe.options.windowContext.document.createElement(listNode.nodeName);
        ...
        if (this.queryState()) {
          ...
          scribe.transactionManager.run(function () {
            if (listItemElement) {
              ...
              var documentFragment = scribe.options.windowContext.document.createDocumentFragment();
